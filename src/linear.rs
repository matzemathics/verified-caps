...
impl LinSystem {
    ...
    #[cfg(bla)]
    fn stuff() {
        assert(self.valid(next_link));

        proof! { lemma_next_link_insert(self, child_link); };
        assert(horizontal_keys(self, child_link) =~= horizontal_keys(self, next_link).insert(0, new));
        ...
        proof! {
            ...
            implies
                self.map@[key@] == #[trigger] old(self).map@[key@] && self@[key] == old(self)@[key]
            by { };
            ...
            assert(horizontal_keys(self, child_link) == horizontal_keys(self, next_link).insert(0, new));
            assert(self@[parent].0 == old(self)@[parent].0);
            assert(self@[parent].1 == horizontal_keys(self, next_link).insert(0, new));
            assert(self@[new] == (data, Seq::<LinKey>::empty()));
        };
    }
}
