...
impl Heap {
    ...
    spec fn hole_at(&self, hole: usize) -> bool {
        self.indices_match() &&
        forall |index: usize| index != hole && index < self@.len() ==> #[trigger] self@[index as int].is_init()
    }

    spec fn wf(&self) -> bool {
        self.indices_match() &&
        forall |index: usize| index < self@.len() ==> #[trigger] self@[index as int].is_init()
    }
